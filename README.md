...
`engine/__init__.py` - currently **1.0.0**. The startup banner, `CHANGELOG.md`,
and the handover docs all reference this one value. (The inline `v0.x.y` notes
in the source code are historical change annotations marking when a fix landed -
see `CHANGELOG.md` - not the current version.)

The project uses **semantic versioning**, `MAJOR.MINOR.PATCH`:

- **MAJOR** - incompatible changes to the output schema (`MASTER_FIELDS`) or the
  `brands.json` format. Existing masters or brand configs may need changes.
- **MINOR** - backward-compatible additions (a new brand, a new column, a new
  transform). Existing configs keep working.
- **PATCH** - backward-compatible bug fixes only.

**Rules for updating the version:**

1. Decide the bump - MAJOR, MINOR, or PATCH - using the rules above.
2. Edit `__version__` in `engine/__init__.py` (this is the only place to change
   the number).
3. Add a dated entry to `CHANGELOG.md` describing the change.
4. Commit, e.g. `git commit -m "Release v1.0.1"`, optionally tag it
   `git tag v1.0.1`, and push to `main`.
5. Railway rebuilds and redeploys on push; confirm the banner shows the new
   version.
